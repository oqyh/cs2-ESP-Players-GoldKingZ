...
public static class Extension
{
    ...
    public static CCSPlayerController? Get_CCSPlayerController_ControlledBot(this CCSPlayerController? player)
    {
        if (player == null || !player.IsValid) return null;

        var csPawn = player.Pawn?.Value?.As<CCSPlayerPawn>();
        if (csPawn != null && csPawn.IsValid)
        {
            var originalController = csPawn.OriginalController?.Value?.As<CCSPlayerController>();
            if (originalController != null && originalController.IsValid && originalController.IsBot)
            {
    ...
    public static bool IsAlive(this CCSPlayerController? player, bool AlsoIncludeControllingBot = false)
    {
        if (player == null || !player.IsValid) return false;

        if (AlsoIncludeControllingBot)
        {
            var targetController = player.Get_CCSPlayerController_ControlledBot();
            if (targetController == null || !targetController.IsValid) return false;

            var pawn = targetController.PlayerPawn.Value;
